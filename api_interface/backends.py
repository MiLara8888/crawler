...
from urlopen_app.models import User
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        # 'Authorization': 'JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJhcGlfaW50ZXJmYWNlIiwidXNlcl9pZCI6MSwiZXhwIjoxNjc5NDA1Mjg3LCJ0eXBlIjoicmVmcmVzaCJ9.dVXg_-BwurmVtsSroVUysHZFFmVVGFseLWzUXwrxYlM',
        # 'User-Agent': 'PostmanRuntime/7.31.1', 'Accept': '*/*', 'Postman-Token': 'f5c09740-4568-4038-9ee0-7d782abd088a',
        # 'Host': '127.0.0.1:4333', 'Accept-Encoding': 'gzip, deflate, br', 'Connection': 'keep-alive', 'Cookie': 'csrftoken=E8TNshv5EqWdlCJV759cvKRy3RJ6kEq5'
        auth_header = authentication.get_authorization_header(request).split()    #строка делится на две части JWT и сам токен авторизации
        auth_header_prefix = self.authentication_header_prefix.lower()    #извлекается JWT

        if not auth_header:    #TODO Обработать NONE
            return None
    ...
    def _authenticate_credentials(self, request, token):
        ...
        try:
            ...
            payload = jwt.decode(token, JWT_SECRET, algorithms=["HS256"])    #раскодирование токена
        except Exception:
